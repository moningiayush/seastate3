...
def load_and_clean(cfg: Config) -> pd.DataFrame:
    ...
    # Keep only relevant columns (bearing optional)
    keep_cols = [cfg.trip_col, cfg.lon_col, cfg.lat_col, cfg.speed_col, cfg.time_col]
    if cfg.bearing_col in df.columns:
        keep_cols.insert(4, cfg.bearing_col)
